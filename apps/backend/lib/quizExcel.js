...
async function upsertQuizWithQuestions({ quiz, questions, forceOverwrite, prisma }) {
...
async function importQuizzesFromWorkbook({ workbook, prisma, forceOverwrite = false, dryRun = false }) {
  ...
  if (dryRun) {
    return {
      ...
      questionRows: rawQuestions.length,
      summary: quizzes.map((quiz) => ({ slug: quiz.slug, title: quiz.title })),
    };
  }

  const results = [];
  for (const quiz of quizzes) {
    const outcome = await upsertQuizWithQuestions({
      quiz,
    ...
    results.push({
      slug: quiz.slug,
      insertedQuestions: outcome.insertedQuestions,
...
function createTemplateWorkbook() {
  ...
  instructions.addRow([""]);
  instructions.addRow([
    "1. Fill the 'Quiz' sheet with each quiz you want to add.",
  ]);
  instructions.addRow([
    "2. Add a matching 'quiz_slug' to every question in the 'Questions' sheet.",
  ]);
  instructions.addRow([
    "3. Keep at least two option columns and make sure 'correct_option' matches one of them.",
  ]);
  instructions.addRow([
    "4. Once ready, upload the file through the admin portal to insert into the database.",
  ]);
